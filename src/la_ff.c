...
static inline cf32_t *normalize_sparse_matrix_row(
        cf32_t *row,
        const len_t os,
        const len_t len
        )
        ...
        )
{
    ...
    hm_t *dt    = (hm_t *)malloc((unsigned long)(ncols-np+3) * sizeof(hm_t));
    ...
    if (j == 0) {
        ...
        dt    = realloc(dt, (unsigned long)(j+3) * sizeof(hm_t));
        ...
        )
{
    ...
    for (i = *pc; i < ncl; ++i) {
        ...
        /* found reducer row, get multiplier */
        const int64_t mul = mod - dr[i];
        const cf32_t *cfs   = gbcf_ff[pivs[i][0]];
...
static cf32_t *reduce_dense_row_by_known_pivots_17_bit(
        int64_t *dr,
        hm_t *const *pivs,
        const hl_t dpiv    /* pivot of dense row at the beginning */
        )
...
static cf32_t *reduce_dense_row_by_known_pivots_31_bit(
        int64_t *dr,
        hm_t *const *pivs,
        const hl_t dpiv    /* pivot of dense row at the beginning */
        )
...
static hm_t **probabilistic_sparse_reduced_echelon_form(
        hm_t **mat
        )
{
    len_t i, j, k, l, m;
...
static hm_t **exact_sparse_reduced_echelon_form(
        hm_t **mat
        )
{
    len_t i, j, k;
...
static cf32_t **sparse_AB_CD_linear_algebra_ff(
        hm_t ***matp
        )
{
    hm_t **mat  = *matp;
...
static cf32_t **interreduce_dense_matrix(
    cf32_t **dm
    )
{
    len_t i, j, l;
...
static cf32_t **probabilistic_sparse_dense_echelon_form(
        hm_t ***matp
        )
{
    len_t i, j, k, l, m;
...
static cf32_t **exact_dense_linear_algebra_ff(
        cf32_t **dm,
        const len_t nr
        )
{
    ...
    cf32_t **tbr  = (cf32_t **)calloc((unsigned long)nr, sizeof(cf32_t *));
    int64_t *dr   = (int64_t *)malloc(
...
static cf32_t **probabilistic_dense_linear_algebra_ff(
        cf32_t **dm,
        const len_t nr
        )
{
 ...
 * linear algebra */
static hm_t **probabilistic_sparse_linear_algebra_ff(
        hm_t **mat,
        stat_t *st
        )
{
    ...
    tmpcf_ff = realloc(tmpcf_ff, (unsigned long)nrl * sizeof(cf32_t *));
...
static hm_t **exact_sparse_linear_algebra_ff(
        hm_t **mat,
        stat_t *st
        )
{
...
static hm_t **exact_sparse_dense_linear_algebra_ff(
        hm_t **mat,
        stat_t *st
        )
{
    ...
    cf32_t **dm;
    dm  = sparse_AB_CD_linear_algebra_ff(&mat);
    if (npivs > 0) {      
        dm  = exact_dense_linear_algebra_ff(dm, npivs);
...
static hm_t **probabilistic_sparse_dense_linear_algebra_ff_2(
        hm_t **mat,
        stat_t *st
        )
{
    ...
    dm  = sparse_AB_CD_linear_algebra_ff(&mat);
    if (npivs > 0) {      
        dm  = probabilistic_dense_linear_algebra_ff(dm, npivs);
        dm  = interreduce_dense_matrix(dm);
    }
...
static hm_t **probabilistic_sparse_dense_linear_algebra_ff(
        hm_t **mat,
        stat_t *st
        )
{

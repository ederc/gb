        ...
        )
{
    ...
    const int16_t fc16  = (int16_t)fc;
    const int32_t inv   = mod_p_inverse_16(row[0], fc16);
        ...
        )
{
    ...
    cf16_t *cf  = (cf16_t *)malloc((unsigned long)(ncols-np) * sizeof(cf16_t));
    j = 0;
    hm_t *rs = row + 3;
    for (i = ncl; i < ncols; ++i) {
        if (dr[i] != 0) {
            ...
            while (bctr < nrbl) {
                ...
                do {
                    ...
                    * threads might directly use the new pivot once it is synced. */
                    if (mat->cf_16[npiv[0]][0] != 1) {
                        normalize_sparse_matrix_row_ff_16(
                                mat->cf_16[npiv[0]], npiv[1], npiv[2], st->fc);
                    }
                    cfs = mat->cf_16[npiv[0]];
                    sc  = npiv[3];
    ...
    )
{
    ...
    for (i = ncr-1; i > -1; --i) {
        if (dm[i]) {
            ...
            dm[i] = reduce_dense_row_by_dense_new_pivots_ff_16(
                    dr, &i, dm, ncr, fc);
        }
    }
    free(dr);
        ...
        )
{
    ...
    for (i = ncr-1; i > -1; --i) {
        if (dm[i] != NULL) {
            cfs = malloc((unsigned long)(ncr-i) * sizeof(cf16_t));
        ...
        )
{
    ...
    for (i = 0; i < nrows; ++i) {
        ...
        mat->cf_16[i] = (cf16_t *)malloc(
                (unsigned long)mat->r[i][2] * sizeof(cf16_t));
        memcpy(mat->cf_16[i], bs->cf_16[mat->r[i][0]],
                (unsigned long)mat->r[i][2] * sizeof(cf16_t));
        pivs[mat->r[i][3]][0] = i;
    }

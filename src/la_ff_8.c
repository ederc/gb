        ...
        )
{
    ...
    const int8_t fc8  = (int8_t)fc;
    const int32_t inv = mod_p_inverse_8(row[0], fc8);
        ...
        )
{
    ...
    cf8_t *cf  = (cf8_t *)malloc((unsigned long)(ncols-np) * sizeof(cf8_t));
    j = 0;
    hm_t *rs = row + 3;
    for (i = ncl; i < ncols; ++i) {
        if (dr[i] != 0) {
    ...
    if (j == 0) {
        ...
        cf      = realloc(cf, (unsigned long)j * sizeof(cf8_t));
        row[0]  = tmp_pos;
        row[1]  = j % 4;
        row[2]  = j;
        mat->cf_8[tmp_pos]  = cf;
            ...
            while (bctr < nrbl) {
                ...
                do {
                    ...
                    * threads might directly use the new pivot once it is synced. */
                    if (mat->cf_8[npiv[0]][0] != 1) {
                        normalize_sparse_matrix_row_ff_8(
                                mat->cf_8[npiv[0]], npiv[1], npiv[2], st->fc);
                    }
                    cfs = mat->cf_8[npiv[0]];
                    sc  = npiv[3];
    ...
    )
{
    ...
    for (i = ncr-1; i > -1; --i) {
        if (dm[i]) {
            ...
            dm[i] = reduce_dense_row_by_dense_new_pivots_ff_8(
                    dr, &i, dm, ncr, fc);
        }
    }
    free(dr);
        ...
        )
{
    ...
    for (i = ncr-1; i > -1; --i) {
        if (dm[i] != NULL) {
            cfs = malloc((unsigned long)(ncr-i) * sizeof(cf8_t));

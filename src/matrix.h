...
static inline void poly_to_sparse_compact_matrix_multiline_row(const gb_t *basis,
    const gb_t *sf, const mpp_t *mpp_start, const nelts_t ncl, const nelts_t nc, const nelts_t idx,
...
static inline void poly_to_sparse_compact_matrix_row_new(const gb_t *basis,
    const gb_t *sf, const mpp_t *mpp_start, const nelts_t ncl, const nelts_t nc, const nelts_t idx,
    const nelts_t bs, src_t **rows)
{
...
static inline void poly_to_sparse_compact_matrix_row_pos_val(const gb_t *basis,
    const gb_t *sf, const mpp_t *mpp_start, const nelts_t ncl, const nelts_t nc, const nelts_t idx,
    const nelts_t bs, src_t **rows)
{
  src_t *bf = (src_t *)malloc(nc * sizeof(src_t));
  ...
  #pragma omp parallel num_threads(t)
  {
    ...
    for (nelts_t i=0; i<nr; i=i+BLOCK) {
      #pragma omp task
      {
        ...
        poly_to_sparse_compact_matrix_multiline_row(basis, sf, sel->mpp+i, mat->ncl, mat->ncl + mat->ncr, i, bs, mat->row+i/2);
      }
    }
    }
  }
  ...
  #pragma omp parallel num_threads(t)
  {
    ...
    for (nelts_t i=0; i<mat->nr; i=i+BLOCK) {
      #pragma omp task
      {
        ...
        //printf("mat->nr %u | i %u | bs %u\n", mat->nr, i, bs);
        //printf("constructs row %u\n",i);
        poly_to_sparse_compact_matrix_row_pos_val(basis, sf, sel->mpp+i, mat->ncl, mat->ncl + mat->ncr, i, bs, mat->row+i);
      }
    }
    }
  }
  ...
  #pragma omp parallel num_threads(t)
  {
    ...
    for (nelts_t i=0; i<mat->nr; i=i+BLOCK) {
      #pragma omp task
      {
        ...
        //printf("mat->nr %u | i %u | bs %u\n", mat->nr, i, bs);
        //printf("constructs row %u\n",i);
        poly_to_sparse_compact_matrix_row_new(basis, sf, sel->mpp+i, mat->ncl, mat->ncl + mat->ncr, i, bs, mat->row+i);
      }
    }
    }
  }
  ...
  #pragma omp parallel num_threads(t)
  {
    ...
    for (nelts_t i=0; i<mat->nr; i=i+BLOCK) {
      #pragma omp task
      {
        ...
        //printf("mat->nr %u | i %u | bs %u\n", mat->nr, i, bs);
        //printf("constructs row %u\n",i);
        poly_to_sparse_compact_matrix_row(basis, sf, sel->mpp+i, mat->ncl, mat->ncl + mat->ncr, i, bs, mat->row+i);
      }
    }
    }
  }
...
static inline sr_t *reduce_lower_by_one_upper_row(sr_t *row, const sr_t *piv, const cf_t mod, const nelts_t ncl, const nelts_t nc)
{
...
static inline src_t *normalize_row_c(src_t *row, const cf_t mod)
{
  ...
  i = i & 1 ? 4 : 2;
  while (i<row[0]) {
    row[i]   = MODP((bf_t)(row[i]) * cinv, mod);
    row[i+2] = MODP((bf_t)(row[i+2]) * cinv, mod);
    i +=  4;
  }
  // possibly not set in the unrolled loop above
  row[2] = 1;
...
#pragma omp parallel for num_threads(nthreads)
  for (nelts_t i=0; i<j; ++i)
    compute_new_pivots_c(mat->row[i], pivs, mat->nr, shift, mat->ncr, mat->mod);
#if newred
  printf("FINAL PIVOTS:\n");
  for (nelts_t i=0; i<mat->ncr; ++i) {
    printf("piv[%u] (%p)",i, pivs[i]);
...
static inline src_t *reduce_lower_by_upper_multiline_rows_c(src_t *row, const smc_t *pivs, const nelts_t idx)
{
  if (row[1] >= pivs->ncl)
    return row;
...
static inline src_t *reduce_lower_by_upper_rows_new_c(src_t *row, const smc_t *pivs, const nelts_t idx)
{
  if (row[1] >= pivs->ncl)
    return row;
...
static inline src_t *reduce_lower_by_upper_rows_pos_val_c(src_t *row, const smc_t *pivs, const nelts_t idx)
{
  if (row[1] >= pivs->ncl)
    return row;
